...
def fetch_weather(game_pk: int, venue_id: int) -> dict[str, Any]:
    ...
    Query order:
    1. ``game_weather`` table (canonical post-PR #4 migration).
    2. ``weather`` table (legacy fallback).
    3. Hard-coded default dict.

    Column names from ``game_weather`` are normalised to the keys that
    :func:`apply_weather_adjustments` expects.
    ...
    # ── 1. game_weather table (correct table from PR #4 migration) ────────────
    try:
        rows = _get("/game_weather", params={"game_pk": f"eq.{game_pk}", "select": "*"})
        if rows:
            raw = rows[0]
            # Normalise column names to what apply_weather_adjustments() expects
            return {
